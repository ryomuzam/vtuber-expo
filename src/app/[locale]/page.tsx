...
import { getSocialLinks, getVenueMapData, getEventScheduleData, getSponsorPageData } from "@/lib/data";
...
export default async function HomePage({ params }: Props) {
  ...
  const [socialLinks, venueMap, eventSchedule, sponsorPage] = await Promise.all([
    getSocialLinks(),
    getVenueMapData(),
  ...
  return (
    <main className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden bg-[#0a0a1a] px-4 text-center">
      ...
      {sponsorPage.isPublic && (
        <div className="w-full">
          <SponsorsSection data={sponsorPage} />
        </div>
      )}
    </main>
  );
